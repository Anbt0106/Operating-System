...
int firstFitSearch(int k) {
    ...
    for (i = 0; i <= MAXPAGES - k; i++) {
        for (j = 0; j < k; j++)
            if (getBit(i + j)) break; // if bit is 1, break the loop
        if (j == k) return i; // if all k bits are 0, return the position
    }
...
int bestFitSearch(int k) {
    ...
    for (i = 0; i <= MAXPAGES - k; i++) {
        ...
        if ((count >= k) && (count < minCount)) {
            minCount = count;  // kích thước nhỏ nhất tìm được
            pos = i; // vị trí bắt đầu của kích thước nhỏ nhất
        }
        i += count;
    }
    return (pos);
...
int worstFitSearch(int k) {
    ...
    for (i = 0; i <= MAXPAGES - k; i++) {
        ...
        if ((count >= k) && (count > maxCount)) {
            maxCount = count;  // kích thước lớn nhất tìm được
            pos = i; // vị trí bắt đầu của kích thước lớn nhất
        }
        i += count;
    }
    return (pos);
    ...
    while (1) {
        ...
        if (((name >= 'a') && (name <= 'z')) ||
            ...
            if (pages > 0) {
                ...
                switch (err) {
                    case -1:
                        fprintf(stderr, "Process %c is already in memory\n", name);
                        break;
                    case -2:
