...
static linkList_t memList; // memory list
...
int firstFitSearch(linkList_t *pList, int pages) {
    ...
    if (pList == NULL) return (-1); // pList is NULL
    pList->pCurrentNode = pList->pFirstNode;
    while (pList->pCurrentNode != NULL) {        // search till the end
        if ((pList->pCurrentNode->label == 'H') && (pList->pCurrentNode->numPages >= pages))
            break;
        pList->pCurrentNode = pList->pCurrentNode->pNext;
    }
...
void printList(linkList_t *pList) {
    ...
    pList->pCurrentNode = pList->pFirstNode;
    while (pList->pCurrentNode != NULL) {
        printf(" %c %c %3d %2d |", pList->pCurrentNode->processName, pList->pCurrentNode->label,
               pList->pCurrentNode->startPage, pList->pCurrentNode->numPages);
        pList->pCurrentNode = pList->pCurrentNode->pNext;
    }
    printf("\n");
    ...
    while (1) {
        ...
        if (((name >= 'a') && (name <= 'z')) ||
            ...
            if (pages > 0) {
                ...
                switch (err) {
                    case -1:
                        fprintf(stderr, "Process %d is already in memory\n", name);
                        break;
                    case -2:
                        fprintf(stderr, "Memory is full\n");
                        break;
                    case -3:
                        fprintf(stderr, "Unable to create link list node\n");
                        break;
                }
            } else {
